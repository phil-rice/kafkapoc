...
class FlinkPipelineLiftTest {
    ...
    void e2e_sync_and_async_value_error_retry() throws Exception {
        ...
        ValueEnvelope<String> in1 = new ValueEnvelope<>("domainType1", okId, "foo", 100, rkd1);
        RawKafkaData rkd2 = new RawKafkaData("err-enrichment", "key-2", 2, 1002L, 1693651200000L);
        ValueEnvelope<String> in2 = new ValueEnvelope<>("domainType2", errEnrich, "Error:enrichment-data2", 200, rkd2);
        RawKafkaData rkd3 = new RawKafkaData("rt-cepEnrichment", "key-3", 2, 1003L, 1693651200000L);
        ValueEnvelope<String> in3 = new ValueEnvelope<>("domainType3", rtCep, "Retry:cepEnrichment-data3", 300, rkd3);
        RawKafkaData rkd4 = new RawKafkaData("err-validate", "key-4", 2, 1004L, 1693651200000L);
        ValueEnvelope<String> in4 = new ValueEnvelope<>("domainType4", errValid, "Error:validate-data4", 400, rkd4);
        RawKafkaData rkd5 = new RawKafkaData("rt-bizLogic", "key-5", 2, 1005L, 1693651200000L);
        ValueEnvelope<String> in5 = new ValueEnvelope<>("domainType5", rtBiz, "Retry:bizLogic-data5", 500, rkd5);

        var src = env.fromElements(in1, in2, in3, in4, in5);

        var main = FlinkPipelineLift.lift(src, repoClass, RETRIES, ERRORS, 10, false, 10_000);
