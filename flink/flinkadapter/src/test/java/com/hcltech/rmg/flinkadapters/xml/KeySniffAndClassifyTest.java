 ...
 * Reads keyPath from AppContainer.resolve("test") and generates XML accordingly.
 */
public final class KeySniffAndClassifyTest {

    private static final OutputTag<ErrorEnvelope<Object, ?>> SNIFF_ERRORS =
            new OutputTag<>("sniff-errors") {};

    // --- helpers ---------------------------------------------------------------

    private static List<String> testKeyPath() {
        IAppContainer c = AppContainer.resolve("test");
    ...
    void happyPath_emitsTuple() throws Exception {
        var fn = new KeySniffAndClassify<Object>("test", SNIFF_ERRORS, 0);
        ...
        try (OneInputStreamOperatorTestHarness<RawMessage, Tuple2<String, RawMessage>> h =
            ...
            ConcurrentLinkedQueue<StreamRecord<ErrorEnvelope<Object, ?>>> side = h.getSideOutput(SNIFF_ERRORS);
    ...
    void missingPath_goesToSideOutput() throws Exception {
        ...
        try (OneInputStreamOperatorTestHarness<RawMessage, Tuple2<String, RawMessage>> h =
            ...
            ValueEnvelope<Object, ?> ve = err.envelope();
            assertNull(ve.header());
            assertTrue(ve.data() instanceof RawMessage);
            assertEquals(raw, (RawMessage) ve.data());
