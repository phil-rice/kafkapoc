...
public class ProducerValidationTest {
  ...
  void duplicateExactProducer_isError() {
    ...
    assertTrue(ex.getMessage().contains("a.b"), ex.getMessage());
  }
  ...
  void prefixOverlapProducer_isError() {
    ...
    IllegalArgumentException ex = assertThrows(IllegalArgumentException.class,
        () -> ProducerValidation.validate(nodes, ptc(), ntc));
    assertTrue(ex.getMessage().contains("Overlapping"), ex.getMessage());
    assertTrue(ex.getMessage().contains("a.b"), ex.getMessage());
    assertTrue(ex.getMessage().contains("a.b.c"), ex.getMessage());
  }
  ...
  void distinctProducers_areValid() {
    ...
    assertDoesNotThrow(() -> ProducerValidation.validate(nodes, ptc(), ntc));
  }
  ...
  void errorListsBothConflictingProducers() {
    ...
    var ex = assertThrows(IllegalArgumentException.class,
        () -> ProducerValidation.validate(set(P1, P2), ptc(), ntc));
