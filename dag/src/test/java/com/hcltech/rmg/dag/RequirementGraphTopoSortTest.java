...
public class RequirementGraphTopoSortTest {
    ...
    void singleOwner_AB_then_ABC() {
        ...
        var gens = Topo.topoSortFromNodes(set(AB, ABC), ptc(), ntc).valueOrThrow();
    ...
    void fork_A_to_ABX_and_ACY() {
        ...
        var gens = Topo.topoSortFromNodes(set(A, ABX, ACY), ptc(), ntc).valueOrThrow();
    ...
    void join_B_and_C_into_D() {
        // B→D and C→D
        var gens = Topo.topoSortFromNodes(set(B_OWNER, C_OWNER, D_JOIN), ptc(), ntc).valueOrThrow();
    ...
    void disconnected_subgraphs_sortedIndependently() {
        ...
        var gens = Topo.topoSortFromNodes(set(A, ABX, X1, Y1), ptc(), ntc).valueOrThrow();
        assertEquals(2, gens.size());
    ...
    void noEdges_allInFirstGen() {
        var gens = Topo.topoSortFromNodes(set(X1, Y1), ptc(), ntc).valueOrThrow();
        assertEquals(List.of(Set.of(X1, Y1)), gens);
    }

    @Test
    void duplicateProducer_isError() {
        // AB & AB_DUP both produce "a.b" → validation error via builder
        var errs = Topo.topoSortFromNodes(set(AB, AB_DUP, ABC), ptc(), ntc).errorsOrThrow();
        var msg = String.join("\n", errs);
    ...
    void prefixOverlapProducers_isError() {
        ...
        var errs = Topo.topoSortFromNodes(set(A, AB, ABC), ptc(), ntc).errorsOrThrow();
        var msg = String.join("\n", errs);
    ...
    void cycle_isDetected_afterEdgesDerived() {
        ...
        var errs = Topo.topoSortFromNodes(set(A_NEEDS_C, B_NEEDS_A, C_NEEDS_B), ptc(), ntc).errorsOrThrow();
        var msg = String.join("\n", errs);
        assertTrue(msg.contains("Cycle detected"), msg);
    }
}
