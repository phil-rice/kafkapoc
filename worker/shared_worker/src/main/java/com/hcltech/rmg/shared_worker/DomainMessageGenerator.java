...
public final class DomainMessageGenerator {
    ...
    /**
     * Infinite stream of the same message with the same domainId and monotonically increasing offset.
     */
    public <T> Stream<T> sequentialStream(String domainId, BiFunction<String, Long, T> mapper) {
        AtomicLong offset = new AtomicLong(0);
        Supplier<T> next = () -> mapper.apply(domainId, offset.getAndIncrement());
