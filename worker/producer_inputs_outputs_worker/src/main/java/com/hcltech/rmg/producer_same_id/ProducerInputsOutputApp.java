package com.hcltech.rmg.producer_same_id;
...
import com.hcltech.rmg.shared_worker.DomainMessageGenerator;
...
public class ProducerInputsOutputApp {
    private static final Logger log = LoggerFactory.getLogger(ProducerInputsOutputApp.class);
    ...
    static String inputOutputStringFor(TestDomainMessage msg) {
        return "<test><input>" + TestDomainMessage.xmlCodec.encode(msg).valueOrThrow() + "</input>" +
                "<output><out><count>" + msg.count() + "</count></out></output></test>";
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        String topic = get("kafka.topic", applicationProperties, "input-output-topic");
        ...
        int count = Integer.parseInt(get("generator.count", applicationProperties, "3000000"));
        int domainCount = Integer.parseInt(get("generator.domain.count", applicationProperties, (count / 10000) + ""));
...
//        Codec<TestDomainMessage, String> codec = Codec.clazzCodec(TestDomainMessage.class);
        ...
        try (KafkaProducer<String, String> producer = newKafkaProducer(applicationProperties)) {
            var it = generator.sequentialStream("domId", TestDomainMessage::create).limit(count).iterator();
            ...
            while (it.hasNext()) {
                ...
                long n = input.count();
                String key = String.valueOf(input.domainId());
                String value = inputOutputStringFor(input);
    ...
    private static Properties loadApplicationProperties() {
        ...
        try (InputStream is = ProducerInputsOutputApp.class.getClassLoader().getResourceAsStream("application.properties")) {
