...
class BehaviorBehaviorConfigWalkerTest {

    /**
     * Records the order and type of callbacks for verification.
     */
    static class RecordingVisitorBehavior implements BehaviorConfigVisitor {
    ...
    void walks_full_graph_and_invokes_generic_then_specific() {
        ...
        var v = new RecordingVisitorBehavior();
    ...
    void tolerates_empty_and_nulls() {
        var config = new BehaviorConfig(null);
        var v = new RecordingVisitorBehavior();
        BehaviorConfigWalker.walk(config, v);
        assertEquals(List.of("onConfig"), sorted(v.calls));

        var config2 = new BehaviorConfig(Map.of("emptyEvent", AspectMap.empty()));
        var v2 = new RecordingVisitorBehavior();
        BehaviorConfigWalker.walk(config2, v2);
