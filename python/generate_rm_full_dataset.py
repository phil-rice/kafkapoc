...
def random_address_line(street_words=3):
    streets = ["High", "Station", "Church", "Green", "Park", "Mill", "Grove", "Oak", "King", "Queen", "Victoria", "George", "Bridge"]
...
def build_manualscan_input_xml(event_code, scan_ts, transmission_ts, functional_location_id, site_id, device_id, user_id,
                              include_route=False, route=None, include_aux=False, email=None, mobile=None):
    parts = ["<manualScan>"]
...
def generate_dataset(parcels, output_dir, batch_size=100000, postcode_file="", seed=42, start_date=None, end_date=None):
    random.seed(seed)
    ...
    in_zip_idx = 1
    out_zip_idx = 1
    in_zip = ZipFile(os.path.join(inputs_dir, f"mp_inputs_{in_zip_idx:04d}.zip"), "w", ZIP_DEFLATED)
    out_zip = ZipFile(os.path.join(outputs_dir, f"mp_notifications_{out_zip_idx:04d}.zip"), "w", ZIP_DEFLATED)
    written_in = 0
    written_out = 0

    # Collections for postcode CSVs
    used_postcodes = set()
    postcode_to_location = {}  # postcode -> (add_line1, add_line2, city, county, country)
    ...
    with open(summary_csv_path, "w", newline="", encoding="utf-8") as summary_f:
        ...
        writer.writerow(["uniqueItemId","productCategory","hasEmail","hasMobile","event","emailNotif","smsNotif"])
        ...
        for i in range(parcels):
            ...
            if postcode not in postcode_to_location:
                ...
                add_line2 = "" if random.random() < 0.5 else ("Flat " + str(random.randint(1,20)))
                country = "GB"
                postcode_to_location[postcode] = (add_line1, add_line2, city, county, country)
            ...
            product_id = {"Tracked24":"100","Tracked48":"101","SpecialDelivery09":"109","SpecialDelivery13":"113"}[category]
            ...
            mp_input_xml = build_mailpiece_input_xml(unique_item_id, upu, postcode, barcode_creation_date, product_id)
            ...
            for idx, ev in enumerate(EVENT_FLOW, start=1):
                ...
                manual_xml = build_manualscan_input_xml(ev, scan_ts, tx_ts, func, site, device_id, "test",
                                                       include_route=include_route, route=route_no,
                                                       include_aux=include_aux, email=(email if include_aux else None),
            ...
            for ev in ("EVDAV", "EVGPD", "ENKDN"):
                ...
                writer.writerow([unique_item_id, category, int(has_email), int(has_mobile), ev, int(email_notif), int(sms_notif)])
...
def parse_args():
    p = argparse.ArgumentParser(description="Generate MPEr inputs, notification outputs, and postcode CSVs.")
    p.add_argument("--parcels", type=int, default=100, help="Number of parcels to generate (each has 4 input scans).")
    p.add_argument("--output-dir", type=str, default="out_full", help="Output directory.")
    p.add_argument("--batch-size", type=int, default=100000, help="How many XML files per zip before rotating.")
    p.add_argument("--postcode-file", type=str, default="", help="Optional file of real UK postcodes (one per line).")
    p.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility.")
    p.add_argument("--start-date", type=str, default="", help="Start date YYYY-MM-DD (default now-365d).")
