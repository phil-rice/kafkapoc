...
public interface IEnrichmentAspectExecutor<CepState, Msg> {
    ValueEnvelope<CepState, Msg> execute(ValueEnvelope<CepState, Msg> input);


    static <CepState, Msg> ErrorsOr<IEnrichmentAspectExecutor<CepState, Msg>> create(CepStateTypeClass<CepState> cepStateTypeClass, Configs configs, AspectExecutor<EnrichmentWithDependencies, ValueEnvelope<CepState, Msg>, CepEvent> evaluator) {
