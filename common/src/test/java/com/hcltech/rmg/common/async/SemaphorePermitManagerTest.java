...
class SemaphorePermitManagerTest {

    @Test
    void initialState_isMaxAvailable() {
        PermitManager pm = new SemaphorePermitManager(256);
    ...
    void acquireUpToLimit_thenReject_thenReleaseAllowsAgain() {
        int max = 8;
        PermitManager pm = new SemaphorePermitManager(max);
    ...
    void tryAcquire_isNonBlockingWhenExhausted() {
        PermitManager pm = new SemaphorePermitManager(2);
        assertTrue(pm.tryAcquire());
        assertTrue(pm.tryAcquire());
        assertEquals(0, pm.availablePermits());
    ...
    @Test
    void releaseIsPairedWithAcquire_contract() {
        PermitManager pm = new SemaphorePermitManager(3);
        assertTrue(pm.tryAcquire());
        pm.release();
        assertEquals(3, pm.availablePermits());

        // Documented contract: callers must not over-release.
        assertEquals(3, pm.availablePermits());
    }
}
